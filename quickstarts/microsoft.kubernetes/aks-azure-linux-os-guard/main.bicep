...
@description('Enable Microsoft Defender for Containers (OS Guard functionality)')
  ...
  properties: {
    ...
    securityProfile: {
      defender: enableDefender ? {
        logAnalyticsWorkspaceResourceId: !empty(logAnalyticsWorkspaceId) ? logAnalyticsWorkspaceId : null
        securityMonitoring: {
          enabled: true
        }
      } : null
